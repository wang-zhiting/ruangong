...
<script>
  ...
  export default {
    ...
    methods: {
      reggo() {
        this.$http
          .post('https://fleamarket.fun:8843/user/update?email='+this.email+'&passWord='+this.passWord,{
            code:this.code
          })
          .then(response=>{
      ...
      verifyClick(verify) {
        ...
        if (this.email!='') {
          ...
          this.$http
           .post('https://fleamarket.fun:8843/user/getCode',prom)
            .then(response=>{
                if(response.data.data==true){
      ...
      updataInfo() {
        ...
        this.$http
          .post('https://fleamarket.fun:8843/user/login', prom)
          .then(response => {
